...
public class GraphConfigurationDetail implements ModelObject {
    ...
    public GraphConfigurationDetail(final AbstractProject<?, ?> project,
        ...
        final String value = createCookieHandler(pluginName).getValue(
                request.getCookies());
        List<Integer> initializationListResult = initializeFrom(value);
        if (!initializationListResult.isEmpty()) {
            final File defaultsFile = createDefaultsFile(project, pluginName);
            if (defaultsFile.exists()) {
                final String defaultValue = readFromDefaultsFile(defaultsFile);
    ...
    public void doSave(final StaplerRequest request,
        ...
        try {
            final JSONObject formData = request.getSubmittedForm();
            final String buildCountString = formData.getString("buildCountString");

            int buildCount = 0;
            if (StringUtils.isNotBlank(buildCountString)) {
                buildCount = formData.getInt("buildCountString");
            }
            final String firstDayCountString = formData.getString("firstDayCountString");
            ...
            final String lastDayCountString = formData.getString("lastDayCountString");
            ...
            final String radioConfigType = formData.getString("radioConfigType");
            ...
            if (formData.has("buildStepString")) {
                final String buildStepString = formData.getString("buildStepString");
            ...
            final String value = serializeToString(configType, buildCount, firstDayCount,
    ...
    protected void persistValue(final String value, final StaplerRequest request,
        ...
        final String buildCount = request.getParameter("buildCount");
        if (buildCount != null) {
        ...
        final String buildStep = request.getParameter("buildStep");
        if (buildStep != null) {
        ...
        final Cookie cookie = createCookieHandler("performance").create(
    ...
    private List<Integer> initializeFrom(final String value) {
        ...
        if (!listErrors.isEmpty()) {
            ...
            if (listErrors.get(0) == -1) {
                ...
                final List<Integer> realListErrors = new ArrayList<Integer>(0);
                for (final Integer typeError : listErrors) {
    ...
    public static GregorianCalendar getGregorianCalendarFromString(
        ...
        final GregorianCalendar outCalendar = new GregorianCalendar();
    ...
    private void reset(List<Integer> initializationResult) {
        ...
        for (final Integer errorNumber : initializationResult) {
            if (errorNumber == -1) {
